(function () {
  ...
  function setTheme(name, save) {
    ...
    if (save) try { localStorage.setItem("compubase-theme", name); } catch (e) { /* ignore */ }
  }
  const sw = $("#swatches");
  if (sw) {
  ...
  try { saved = localStorage.getItem("compubase-theme") || saved; } catch (e) { /* ignore */ }
  ...
  if (canvas && window.FlappyCompu) {
    const best = $("#web-best");
    const game = new window.FlappyCompu(canvas, {
  ...
  const bg = $("#flappy-bg");
  if (bg && window.FlappyCompu) {
    const g = new window.FlappyCompu(bg, { hud: false });
