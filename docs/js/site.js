(function () {
  ...
  const url = `https://github.com/${repo}/releases/latest/download/NexusHub-Setup.exe`;
  ...
  const themes = {
    "Nexus Violet": ["#7c6cff", "#22d3ee"], Ocean: ["#3b82f6", "#2dd4bf"], Sunset: ["#f97316", "#ec4899"],
  ...
  function setTheme(name, save) {
    const t = themes[name] || themes["Nexus Violet"];
  ...
  if (board) {
    ...
    if (!cfg.leaderboardDb) {
      ...
      board.innerHTML = `<li class="empty">The global leaderboard opens very soon.<br>Warm up here, then download NexusHub to post your score.</li>`;
    } else {
